...
globals[colors is-stopped steps]

nodes-own [current-cost]
links-own[weight]
...
to setup
  ...
  create-nodes num-nodes [
    ...
    set current-cost cost-of-color color
  ]
...
to go
  ...
  ask nodes [
    if count link-neighbors = 0 [
      show "The graph is not connected. Setting up again..."
      set to-setup true
      set is-stopped true
      stop
    ]
...
to change-color
  ...
  if ctr = 0 and constraint-violations [
    show "No solution"
    set is-stopped true
    stop
  ]
...
to-report conflict-links [the-color]
  report my-links with [
    (end1 != myself and [color] of end1 = the-color) or
    (end2 != myself and [color] of end2 = the-color)]
end

to-report is-endless
  if (count my-links with [weight > num-edges]) > 0 [
    report true
  ]
  report false
end
