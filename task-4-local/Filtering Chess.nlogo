...
to-report new-domain
  let res (n-values (num-nodes - 1) [
    n-of (1 + random ((length colors) - 1)) colors])
  ; CHEAT
  set res fput (list one-of colors) res
  report res
end
...
to-report is-finished
  ...
  if counter = domain-len [ report true ]
  report false
end

to layout
  layout-spring nodes edges 0.2 5 2
  layout-spring mininodes minilinks 0.1 1 1
end
...
to revise [other-node]
  let my-domain item who domain
  let his-who [who] of other-node
  let his-domain item his-who domain

  if (length my-domain = 0) or (length his-domain != 1) [stop]

  let his-pos first his-domain
  let his-occupancy []
  ask node his-who [
    set pos his-pos
    move-to-cell pos
    ;show pos
    set his-occupancy figure-occupied
    show figure-occupied
  ]

  let whos n-values num-nodes [[i] -> (i + 1)]
  foreach his-occupancy [[coords] ->
    if (member? coords my-domain) [
      let my-new-domain remove coords my-domain
