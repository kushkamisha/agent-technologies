...
to value-iteration
  ...
  while [delta > epsilon * (1 - gamma) / gamma][
    set delta 0
    ask patches with [pcolor = black or pcolor = brown] [
      foreach actions [[a] ->
        let x pxcor
        let y pycor
        let best-action 0
...
to-report get-best-action
  ...
  ; If a patch is tanks - the probabilities of moving up/down/left/right/don't move - are equal
...
to-report get-reward
  ...
  if (pcolor = brown) [report tanks-reward]
